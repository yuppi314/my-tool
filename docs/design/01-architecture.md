...
現在の `yuppi314/my-tool` リポジトリには、既に「九星気学×マヤ暦 診断ツール」という**別のWebアプリ**が `src/` 直下に存在しています。本プロジェクト(デスクトップアプリ「MITSU」)を同じリポジトリ内で開発する場合、既存の `src/` と衝突しないよう、**モノレポ構成(`apps/mitsu/` 配下に新規配置)** で進める前提としています(詳細は [03-ディレクトリ構成](./03-directory-structure.md))。別リポジトリに分けたい場合はお知らせください。
