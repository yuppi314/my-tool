...
### Renderへのデプロイ(無料プランでまず試す場合)

このリポジトリは九星気学ツールと開業サポートツールが同じサーバー(同じRenderサービス)で動くため、
デプロイ手順は上の「無料で最短公開する」と同じです。追加で `KAIGYO_TOKEN` を設定する点だけ異なります。

1. [Render.com](https://render.com) にGitHubアカウントでサインアップ(無料・カード登録不要)。
2. ダッシュボードで「New +」→「Blueprint」→ `yuppi314/my-tool` を選択。
3. `render.yaml` が読み込まれ、環境変数の入力を求められます。`BASE_URL` はRenderが発行するURL、
   `ADMIN_TOKEN` と `KAIGYO_TOKEN` は好きな合言葉(他人に推測されにくいもの)を入力してください。
   `KAIGYO_TOKEN` が、開業サポートツールにログインするときの合言葉になります。
4. 数分待つとURLが発行されます。`https://発行されたURL/kaigyo/login.html` にアクセスし、
   設定した `KAIGYO_TOKEN` でログインしてください。

**注意(無料プランの制限)**: 無料プランは永続ディスクが使えないため、しばらくアクセスがないと
サービスが休止し、次にアクセスしたときに再起動されます。このとき `data/app.db` の中身(入力した
プロフィール・やることリストの進み具合・書類の下書き・補助金の検討リスト)は**リセットされて消えます**。
数日〜数週間にわたって使い続けたい場合は、上の「本格運用する場合(Docker + 永続ディスク)」と同じ要領で
`render.yaml` の `runtime` を `docker` に、`plan` を `starter` 以上に変更し、
`disk`(`mountPath: /app/data`)を追加した構成に切り替えてください(有料プランが必要です)。
